...
ENV SET_MISC_NGINX_MODULE_VERSION 0.31
...
RUN CONFIG="\
        ...
        --add-module=/usr/local/nginx/modules/lua_nginx_module \
    " \
    ...
    && mkdir -p /usr/local/nginx/modules/lua_nginx_module \
    && tar -zxC /usr/local/nginx/modules/lua_nginx_module --strip-components=1 -f lua_nginx_module.tar.gz \
    && rm lua_nginx_module.tar.gz \

    #install luajit
    && curl -fSL http://luajit.org/download/LuaJIT-$LUA_JIT_VERSION.tar.gz -o lua_jit.tar.gz \
    && mkdir -p /usr/src \
    && tar zxC /usr/src -f lua_jit.tar.gz \
    && rm lua_jit.tar.gz \
    && cd /usr/src/LuaJIT-$LUA_JIT_VERSION \
    && make \
    && make install \
    && cd .. \
    && rm -rf /usr/src/LuaJIT-$LUA_JIT_VERSION \
